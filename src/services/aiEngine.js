...
// }
/**
 * BrandMind — Secure AI Engine
 *
 * The browser NEVER receives the Gemini API key.
 * Requests go to our own /api/gemini backend.
 */
...
export function saveStoredCredentials({ provider }) {
  try {
...
async function callGemini(prompt) {
  ...
  if (!response.ok) {
    throw new Error(
      data?.error || `Gemini request failed with status ${response.status}`
    );
  }
...
export async function executeStagePrompt({
  stageName,
  prompt,
  fallbackFn,
  context,
  ...
  try {
    ...
    return {
      result,
      executionMode: 'live-gemini'
    };
  } catch (err) {
    console.warn(
      `[BrandMind] Gemini failed for "${stageName}":`,
      err.message
    );

    return {
      result: fallbackFn(context, parsedIdea),
